...
tmux send-keys -t $SESSION:4 ' echo" Disabled Temporarily /opt/n1ql/cbq-engine -datastore=http://localhost:8091/"'  Enter
tmux send-keys -t $SESSION:5 ' cd ~/repos/auth-dialogs/ && sh   bin/default.sh'  Enter
tmux send-keys -t $SESSION:6 ' cd ~/repos/admin-dashboard/    && node bin/www'  Enter
tmux send-keys -t $SESSION:7 ' cd ~/repos/user-dashboard/     && node bin/www'  Enter
